...
import moment from 'moment'; // หาก error ให้ติดตั้ง moment โดยใช้ npm install moment (Update By Nop 2/12/2566)
...
import { GetIngredientById, GetIngredientTypes, UpdateIngredient } from "../../../../services/https/ingredient";
...
function IngredientEdit() {
    ...
    const onFinish = async (values: IngredientsInterface & ResourceInterface & HistoryInterface) => {
        ...
        values.Amount = parseInt(values.IngredientAmount!.toString(), 10);
        ...
        let resHistory = await UpdateHistory(values);
        if (resIngredient.status && resResource.status && resHistory.status) {
            messageApi.open({
                type: "success",
            ...
            <Card>
                ...
                >
                    <Row gutter={[16, 16]}>
                        ...
                        <Form.Item
                            label="รูปวัตถุดิบ (ต้องเพิ่มใหม่ทุกครั้งที่ทำการแก้ไข)"
